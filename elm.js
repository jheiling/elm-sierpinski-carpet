...
Elm.Model.make = function (_elm) {
   ...
   $Signal = Elm.Signal.make(_elm);
   var _op = {};
   var isFilled = F2(function (x,y) {
      isFilled: while (true) if (_U.cmp(x,0) > 0 || _U.cmp(y,
      0) > 0) if (_U.eq(A2($Basics._op["%"],x,3),
         1) && _U.eq(A2($Basics._op["%"],y,3),1)) return false; else {
               var _v0 = x / 3 | 0,_v1 = y / 3 | 0;
               x = _v0;
               y = _v1;
               continue isFilled;
            } else return true;
   });
   var defaultModel = {color: $Color.red
                      ,pixelSize: {ctor: "_Tuple2",_0: 5.0,_1: 5.0}};
   var Model = F2(function (a,b) {
      return {color: a,pixelSize: b};
   });
...
Elm.View.make = function (_elm) {
   ...
   $Signal = Elm.Signal.make(_elm);
   var _op = {};
   var pixel = function (model) {
      return A2($Graphics$Collage.filled,
      model.color,
      A2($Graphics$Collage.rect,
      $Basics.fst(model.pixelSize),
      $Basics.snd(model.pixelSize)));
   };
   var view = F2(function (_p0,model) {
      var _p1 = _p0;
      var _p3 = _p1._0;
      var _p2 = _p1._1;
      var offsetY = $Basics.toFloat(0 - _p2) / 2.0;
      var offsetX = $Basics.toFloat(0 - _p3) / 2.0;
      var pixelSizeY = $Basics.snd(model.pixelSize);
      var pixelSizeX = $Basics.fst(model.pixelSize);
      var hMax = $Basics.ceiling($Basics.toFloat(_p2 - 1) / pixelSizeY);
      var wMax = $Basics.ceiling($Basics.toFloat(_p3 - 1) / pixelSizeX);
      var pixel$ = pixel(model);
      var drawPixel = F2(function (x,y) {
         return A2($Graphics$Collage.move,
         {ctor: "_Tuple2"
         ,_0: $Basics.toFloat(x) * pixelSizeX + offsetX
         ,_1: $Basics.toFloat(y) * pixelSizeY + offsetY},
         pixel$);
      });
      var line = function (y) {
         return A3($List.foldr,
         F2(function (x,acc) {
            return A2($Model.isFilled,x,y) ? A2($List._op["::"],
            A2(drawPixel,x,y),
            acc) : acc;
         }),
         _U.list([]),
      ...
      var rows = $List.concat(A3($List.foldr,
      F2(function (y,acc) {
         return A2($List._op["::"],line(y),acc);
      }),
      _U.list([]),
      _U.range(0,hMax)));
      return A3($Graphics$Collage.collage,_p3,_p2,rows);
   });
...
Elm.Main.make = function (_elm) {
   ...
   var _op = {};
   var loadModel = Elm.Native.Port.make(_elm).inbound("loadModel",
   "Maybe.Maybe Main.ModelJS",
   function (v) {
      return v === null ? Elm.Maybe.make(_elm).Nothing : Elm.Maybe.make(_elm).Just(typeof v === "object" && "colorRGBA" in v && "pixelSize" in v ? {_: {}
                                                                                                                                                   ,colorRGBA: typeof v.colorRGBA === "object" && v.colorRGBA instanceof Array ? {ctor: "_Tuple4"
                                                                                                                                                                                                                                 ,_0: typeof v.colorRGBA[0] === "number" && isFinite(v.colorRGBA[0]) && Math.floor(v.colorRGBA[0]) === v.colorRGBA[0] ? v.colorRGBA[0] : _U.badPort("an integer",
                                                                                                                                                                                                                                 v.colorRGBA[0])
                                                                                                                                                                                                                                 ,_1: typeof v.colorRGBA[1] === "number" && isFinite(v.colorRGBA[1]) && Math.floor(v.colorRGBA[1]) === v.colorRGBA[1] ? v.colorRGBA[1] : _U.badPort("an integer",
                                                                                                                                                                                                                                 v.colorRGBA[1])
                                                                                                                                                                                                                                 ,_2: typeof v.colorRGBA[2] === "number" && isFinite(v.colorRGBA[2]) && Math.floor(v.colorRGBA[2]) === v.colorRGBA[2] ? v.colorRGBA[2] : _U.badPort("an integer",
                                                                                                                                                                                                                                 v.colorRGBA[2])
                                                                                                                                                                                                                                 ,_3: typeof v.colorRGBA[3] === "number" ? v.colorRGBA[3] : _U.badPort("a number",
                                                                                                                                                                                                                                 v.colorRGBA[3])} : _U.badPort("an array",
                                                                                                                                                   v.colorRGBA)
                                                                                                                                                   ,pixelSize: typeof v.pixelSize === "object" && v.pixelSize instanceof Array ? {ctor: "_Tuple2"
                                                                                                                                                                                                                                 ,_0: typeof v.pixelSize[0] === "number" ? v.pixelSize[0] : _U.badPort("a number",
                                                                                                                                                                                                                                 v.pixelSize[0])
                                                                                                                                                                                                                                 ,_1: typeof v.pixelSize[1] === "number" ? v.pixelSize[1] : _U.badPort("a number",
                                                                                                                                                                                                                                 v.pixelSize[1])} : _U.badPort("an array",
                                                                                                                                                   v.pixelSize)} : _U.badPort("an object with fields `colorRGBA`, `pixelSize`",
      v));
   });
   var fromModelJS = function (modelJS) {
      var _p0 = modelJS.colorRGBA;
      var r = _p0._0;
      var g = _p0._1;
      var b = _p0._2;
      var a = _p0._3;
      return {color: A4($Color.rgba,r,g,b,a)
             ,pixelSize: modelJS.pixelSize};
   };
   var model = A2($Maybe.withDefault,
   $Model.defaultModel,
   A2($Maybe.map,fromModelJS,loadModel));
   var main = A2($Signal.map,
   A2($Basics.flip,$View.view,model),
   $Window.dimensions);
   var ModelJS = F2(function (a,b) {
      return {colorRGBA: a,pixelSize: b};
   });
   return _elm.Main.values = {_op: _op
                             ,ModelJS: ModelJS
                             ,main: main
                             ,model: model
                             ,fromModelJS: fromModelJS};
};
